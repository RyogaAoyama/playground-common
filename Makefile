...
cfn-lint:
	docker run --mount type=bind,source=$(CURDIR),target=/usr/src --mount type=bind,source=$(HOME)/.aws/,target=/root/.aws playground-common:tools cfn-lint ./cloudformation/Networks.yml
deploy-net:
